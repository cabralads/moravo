...
// =========================================================================
async function enviarTemplateConvite({ telefone, codigoConvite }) {
  ...
  if (!resp.ok) {
    const detalhe = (corpo.error && (corpo.error.error_user_msg || corpo.error.message)) || texto.slice(0, 200);
    const err = new Error(`Meta retornou ${resp.status}: ${detalhe}`);
    err.meta = corpo.error || null;
    throw err;
  }
