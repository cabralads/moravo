...
async function getConfig({ semCache } = {}) {
  ...
  const r = await query(
    `SELECT phone_number_id, waba_id, api_version, token_cifrado,
            template_nome, template_corretor, template_idioma, ativo, atualizado_em,
            template_comprador, template_corretor_atendimento,
            webhook_token, app_secret_cifrado
  ...
  const config = {
    ...
    template_nome:   linha.template_nome || 'convite_grupo_proprietario',
    template_corretor: linha.template_corretor || '',
    template_comprador: linha.template_comprador || '',
    template_corretor_atendimento: linha.template_corretor_atendimento || '',
    template_idioma: linha.template_idioma || 'pt_BR',
    ativo:           !!linha.ativo,
    atualizado_em:   linha.atualizado_em || null,
    token:           decifrar(linha.token_cifrado),
    webhook_token:   linha.webhook_token || '',
    app_secret:      decifrar(linha.app_secret_cifrado),
  };
...
// ---- Status de ENTREGA vindo do webhook da Meta
// Só atualiza linha que já existe: o webhook nunca cria registro. Assim uma
// chamada forjada não consegue inventar envio nenhum, no máximo mexeria num
// wamid que já conhecesse.
//
// A Meta manda os estados fora de ordem com frequência (o 'sent' chegando
// depois do 'delivered', por exemplo). Por isso o status só avança na escala,
// nunca regride, e 'failed' passa na frente de tudo porque é o que importa.
const ORDEM_ENTREGA = { sent: 1, delivered: 2, read: 3, failed: 4 };

async function registrarStatusEntrega({ wamid, entrega, erro, quando }) {
  if (!wamid || !entrega) return { atualizado: 0 };
  const peso = ORDEM_ENTREGA[entrega] || 0;
  const r = await query(
    `UPDATE moravo.whatsapp_envios
        SET entrega      = $2,
            entrega_erro = $3,
            entrega_em   = COALESCE($4, NOW())
      WHERE wamid = $1
        AND COALESCE(CASE entrega
              WHEN 'sent' THEN 1 WHEN 'delivered' THEN 2
              WHEN 'read' THEN 3 WHEN 'failed'    THEN 4 ELSE 0 END, 0) < $5
      RETURNING id`,
    [wamid, entrega, erro || null, quando || null, peso]
  );
  return { atualizado: r.rowCount };
}
