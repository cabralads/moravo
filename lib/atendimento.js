...
async function atribuirCorretor(atendimentoId) {
  ...
  const compradorNome = ((await query(
    `SELECT nome FROM moravo.usuarios WHERE id = $1`, [at.comprador_id]
  )).rows[0] || {}).nome || 'Um comprador';
  ...
  // O dono do imóvel também precisa saber. Ele não entra neste grupo (senão
  // negociaria direto com o comprador e a intermediação sumiria), mas saber
  // que apareceu comprador no imóvel dele é a informação mais importante que
  // ele recebe da plataforma.
  const jaAvisado = await query(
    `SELECT 1 FROM moravo.notificacoes
      WHERE usuario_id = $1 AND tipo = 'comprador_no_seu_imovel'
        AND (payload->>'atendimento_id')::bigint = $2 LIMIT 1`,
    [at.dono_id, atendimentoId]
  );
  if (jaAvisado.rowCount === 0) {
    await criarNotificacao({
      usuario_id: at.dono_id,
      tipo: 'comprador_no_seu_imovel',
      imovel_id: at.imovel_id,
      payload: {
        atendimento_id: atendimentoId,
        imovel_titulo: at.titulo,
        imovel_codigo: at.codigo,
        comprador_nome: compradorNome,
        corretor_nome: escolhido.nome,
      },
    }).catch((err) => console.warn('[atendimento] aviso ao dono falhou:', err.message));
  }
