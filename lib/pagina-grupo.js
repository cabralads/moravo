...
function render({ destino, nome, imovel, papel }) {
  // Dois endereços para o mesmo grupo, e a ordem importa no celular:
  //   whatsapp://chat?code=  abre o aplicativo direto
  //   https://chat.whatsapp.com/  cai na página web do WhatsApp, que dentro do
  //   navegador embutido do próprio WhatsApp acaba oferecendo instalar o app
  //   para quem já tem o app (aconteceu no iPhone, 27/08/2026).
  // O código dos dois sai do mesmo link já validado, então continua sem virar
  // redirecionador aberto.
  // Revalida aqui também: a rota já filtra, mas quem renderiza a página não
  // deveria depender disso para nunca emitir um link para fora do WhatsApp.
  destino = destinoSeguro(destino);
  const codigo = (destino.match(/chat\.whatsapp\.com\/([A-Za-z0-9_-]{4,60})$/) || [])[1] || '';
  const destinoApp = codigo ? 'whatsapp://chat?code=' + codigo : '';
...
<main class="cartao">
  ...
  <a class="btn" href="${escapar(destinoApp || destino)}" data-web="${escapar(destino)}" id="ir">
  ...
  <p class="aviso"><a href="${escapar(destino)}" id="ir-web">Abrir pelo navegador</a></p>
</main>

<script>
  (function () {
    var botao = document.getElementById('ir');
    var app = botao.getAttribute('href');
    var web = botao.getAttribute('data-web');
    var saiu = false;

    // Se o aplicativo abrir, a aba vai para segundo plano. É o único sinal
    // confiável: não existe retorno de "o esquema whatsapp:// funcionou".
    document.addEventListener('visibilitychange', function () {
      if (document.hidden) saiu = true;
    });

    function tentar() {
      if (app === web) { window.location.replace(web); return; }
      window.location.href = app;
      // Não abriu em 2,2s: provavelmente não há app registrado no esquema
      setTimeout(function () {
        if (!saiu && !document.hidden) window.location.replace(web);
      }, 2200);
    }

    botao.addEventListener('click', function (e) { e.preventDefault(); tentar(); });
