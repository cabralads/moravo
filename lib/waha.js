...
async function definirFotoGrupo(grupoJid, caminhoArquivo) {
  const cfg = await config();
  ...
  const arquivo = caminhoArquivo || path.join(__dirname, '..', 'public', 'img', 'moravo-grupo.jpg');
  if (!fs.existsSync(arquivo)) throw new Error('Imagem do grupo não encontrada: ' + arquivo);
  ...
  const corpo = { file: { mimetype: 'image/jpeg', filename: 'moravo.jpg', data: base64 } };
  const candidatos = [
