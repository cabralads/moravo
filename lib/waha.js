...
// ---- Foto do grupo
// O grupo nasce com o avatar genérico do WhatsApp, e quem recebe o convite não
// tem como saber que é da Moravo antes de entrar. A imagem vem do próprio
// repositório, então não depende de o site estar no ar para funcionar.
//
// Falhar aqui não pode derrubar nada: grupo sem foto continua sendo um grupo.
async function definirFotoGrupo(grupoJid) {
  const cfg = await config();
  if (!cfg.apiKey) throw new Error('Chave de API do Waha não configurada.');
  if (!grupoJid) throw new Error('JID do grupo é obrigatório.');

  // A imagem vem do painel quando o admin subiu uma; senão, do repositório.
  const { obterFotoGrupo } = require('./foto-grupo');
  const foto = await obterFotoGrupo();
  if (!foto) throw new Error('Nenhuma imagem de grupo disponível.');
  const base64 = foto.base64;
  const mime = foto.mimetype;

  // A rota muda de nome entre versões do Waha. Mesma estratégia do invite:
  // tenta a que a nossa instância responde primeiro e cai para as outras.
  const jid = encodeURIComponent(grupoJid);
  const corpo = { file: { mimetype: mime, filename: 'moravo', data: base64 } };
  const candidatos = [
    ['PUT',  `${cfg.url}/api/${cfg.sessao}/groups/${jid}/picture`, corpo],
    ['POST', `${cfg.url}/api/${cfg.sessao}/groups/${jid}/picture`, corpo],
    ['PUT',  `${cfg.url}/api/${cfg.sessao}/chats/${jid}/picture`,  corpo],
  ];

  let ultimoErro = '';
  for (const [metodo, url, body] of candidatos) {
    try {
      ...
      const texto = await resp.text();
      console.log(`[waha] ${metodo} ${url} -> ${resp.status}`);
      if (resp.ok) return true;
      ultimoErro = resp.status + ': ' + texto.slice(0, 200);
    } catch (err) {
      ultimoErro = err.message;
    }
  }
  throw new Error('Nenhuma rota de foto respondeu. Último erro: ' + ultimoErro);
}

// ---- Renomeia o grupo
// Usado quando o atendimento do comprador troca de corretor: o nome do grupo
// carrega o primeiro nome dele, e ficar com o nome de quem não entrou seria
// pior do que não ter nome nenhum.
async function renomearGrupo(grupoJid, nome) {
  const cfg = await config();
  if (!cfg.apiKey) throw new Error('Chave de API do Waha não configurada.');
  if (!grupoJid || !nome) throw new Error('Grupo e nome são obrigatórios.');

  const jid = encodeURIComponent(grupoJid);
  const candidatos = [
    ['PUT',  `${cfg.url}/api/${cfg.sessao}/groups/${jid}/subject`, { subject: nome }],
    ['POST', `${cfg.url}/api/${cfg.sessao}/groups/${jid}/subject`, { subject: nome }],
    ['PUT',  `${cfg.url}/api/${cfg.sessao}/chats/${jid}/subject`,  { subject: nome }],
  ];
  let ultimoErro = '';
  for (const [metodo, url, body] of candidatos) {
    try {
      ...
      console.log(`[waha] ${metodo} ${url} -> ${resp.status}`);
      if (resp.ok) return true;
      ultimoErro = resp.status + ': ' + (await resp.text()).slice(0, 200);
    } catch (err) { ultimoErro = err.message; }
  }
  throw new Error('Nenhuma rota de renomear respondeu. Último erro: ' + ultimoErro);
}
