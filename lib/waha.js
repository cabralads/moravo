...
async function diagnostico() {
  const base = {
    url: WAHA_URL,
    sessao_em_uso: WAHA_SESSION,
    sessao_veio_do_env: !!process.env.WAHA_SESSION,
    api_key_definida: !!WAHA_API_KEY,
    api_key_final: WAHA_API_KEY ? '...' + WAHA_API_KEY.slice(-4) : '',
    atendente_principal: (process.env.WAHA_ATENDENTE_PRINCIPAL || '').replace(/\D/g, '') || '',
    participantes_extra: (process.env.WAHA_PARTICIPANTES_EXTRA || ''),
    sessoes: [],
    erro: null,
  };
  ...
  try {
    const resp = await fetch(`${WAHA_URL}/api/sessions`, {
    ...
    const achou = base.sessoes.filter(function (x) { return x.nome === WAHA_SESSION; })[0];
